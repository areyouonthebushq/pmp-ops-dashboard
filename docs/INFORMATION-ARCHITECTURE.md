...
│  │ #loginScreen│     │ #modeScreen  │     │ (#app)  #qcStationShell      │  │
│  │ (if auth on)│     │ Admin|FM|QC  │     │         #floorManagerShell   │  │
│  └─────────────┘     └──────────────┘     └─────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| **Launcher** | Admin, Floor Manager (role-gated, `display:none` by default), QC Station. Last choice restored on refresh; default-to-floor when no last. Press Station launcher and shell purged — see § IA Purgatory. |
| **Admin app** | Nav: FLOOR · JOBS · TODOS · LOG · NOTES; CREW, PVC, DEV, AUDIT via utility menu. ENGINE has no nav item — reached only via clock click (`#clock` → `goPg('engine')`). AUDIT hidden from main nav (utility menu). SHIP purged — see § IA Purgatory and `docs/purgatory-protocol.md`. FAB on Floor/Jobs = New Job. |
...
flowchart TB
  ...
  subgraph Purged["Purged (see § IA Purgatory)"]
    Sh["◈ SHIP (purged)"]
  end
  subgraph Hidden["Hidden from main nav"]
    A["▣ AUDIT (utility menu)"]
  end
  subgraph Stations
    QS[QC Station]
    FM[Floor Manager]
...
| **Dev notes** | `S.devNotes[]`, Supabase `dev_notes` | area, text, person, timestamp; optional stage, type, entity (DEV 2.0 rails) |
...
```
  JOB ◄──────────────────────────────────────────────────────────────┐
   ...
   ├── fulfillment_phase  (text)  ── persists; SHIP page purged         │
  ...
  dev_notes ──► area, text, person, timestamp; optional stage, type, entity (DEV 2.0 rails)
```
...
| ship | — | **Purged.** No `pg-ship` in DOM; no `renderShip()`. Formerly: jobs grouped by fulfillment_phase. See § IA Purgatory and `docs/purgatory-protocol.md`. |
| engine | pg-engine | ENGINE metrics/console. No nav item; reached only via clock click (`#clock` → `goPg('engine')`). |
...
| dev | pg-dev | DEV page: area select, three rails (stage, type, entity — DEV 2.0), add note, dev feed (admin only). Constants: `DEV_STAGES`, `DEV_WORK_TYPES`, `DEV_ENTITIES` in core.js. |

### 5.2 Launcher and stations

| Surface | id / trigger | Purpose |
|---------|----------------|--------|
| Launcher | modeScreen | Admin, Floor Manager (role-gated), QC Station; Last: OPEN; Sign out. Press Station launcher and shell purged — see § IA Purgatory. |
| Press Station | — | **Purged.** Formerly: single-press logging shell. See § IA Purgatory and `docs/purgatory-protocol.md`. |
...
| Import Review | importReviewWrap | Staged import (CSV/photo/PDF) → review table → CREATE JOBS. Staged import with review: see `IMPORT-REVIEW-ARCHITECTURE.md`. |
...
**Status:** Purged. No `pg-ship` in DOM; no `renderShip()`; no case in `renderAdminShell()`. See § IA Purgatory and `docs/purgatory-protocol.md`.

The former SHIP page grouped jobs by `fulfillment_phase`. That workflow is now covered by: LOG SHIP actions (BOXED/READY/QUACK for counted movement), Card Zone PACKING face (readiness), JOBS LIVE column (at-a-glance status), and the RSP fulfillment-phase dropdown. The `fulfillment_phase` field and `setFulfillmentPhase()` remain in use.

---

### 6.7 DEV — One-sheet (incl. DEV 2.0)

| Attribute | Detail |
|-----------|--------|
| **DEV 2.0 three rails** | Stage, type, entity. Constants in `core.js`: `DEV_STAGES`, `DEV_WORK_TYPES`, `DEV_ENTITIES`. Dev notes may include optional fields `stage`, `type`, `entity` alongside area, text, person, timestamp. |

---

### 6.8 Right-Side Panel (RSP) — One-sheet
...
3. Admin → enter app, nav default FLOOR. Floor Manager / QC → enter station shell.
...
│  [email] [password]         [Admin] [Floor Manager] [QC Station]           │
...
│  pg-crew        │
│  pg-dev         │
│  (pg-ship purged; pg-audit hidden) │
│  FAB [+]        │
└────────┬────────┘
         │ overlays (when open)
...
| **SHIP** | Former fulfillment-phase page. **Purged.** See § IA Purgatory and `docs/purgatory-protocol.md`. |
...
| **Fulfillment phase** | Job field (`fulfillment_phase`). Formerly used by SHIP page for grouping. Field persists in data model; SHIP page purged. |
...
## 12. IA Purgatory — Decommissioned Surfaces and Paths

**SHIP page.** Former fulfillment-phase grouping page. Purged. The `fulfillment_phase` field and RSP dropdown remain in use. LOG SHIP actions (BOXED/READY/QUACK) and Card Zone PACKING cover the workflow. See `docs/purgatory-protocol.md` (SHIP entry).

**Press Station.** Former launcher choice and shell for single-press logging. Purged. LOG console (6-action) and Floor press grid cover the workflow. See `docs/purgatory-protocol.md`.

**Historical nav (v0).** An earlier IA had QC as a top-level page and Press 1–4 in the launcher; current product uses LOG (6-action) and QC Station shell. See `OLD_INFORMATION-ARCHITECTUREv0-archive.md` for the historical record.

---

*End of Information Architecture. For state and implementation details see STATE-SNAPSHOT.md and code.*
